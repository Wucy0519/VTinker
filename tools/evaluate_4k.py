...
dataset_name = "xiph4k" # Choose from ["xiph4k", "davis4k"]
...
metrics = ["psnr", "ssim", "lpips", "dists", "ie"] # Choose your metrics
...
def inferRGB(*inputs):
    ...
    for time in range(SCALE - 1):
        ...
        with torch.no_grad():
            img = model.inference(img0.half(), img1.half(), t, flownet_deep=flow_deep, skip_num=skip_num)
        tenOut = torch.clamp(img.float(), 0, 1)
        tenOut = tenOut[:, :, :h, :w]
