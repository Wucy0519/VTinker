...
def global_correlation_softmax(feature0, feature1,
    ...
    ones = torch.ones(b,2,h,w).view(b, 2, -1).permute(0, 2, 1).to(correlation.device)
    ...
    if pred_bidir_flow:
        ...
        ones = ones.repeat(2, 1, 1)  # [2*B, H*W, 2]
        b = b * 2

    prob = F.softmax(correlation, dim=-1)  # [B, H*W, H*W]

    # correspondence = torch.matmul(prob, ones).view(b, h, w, 2).permute(0, 3, 1, 2)  # [B, 2, H, W]
    # correspondence = correspondence - correspondence.detach() + torch.matmul(prob, grid).view(b, h, w, 2).permute(0, 3, 1, 2).detach()
