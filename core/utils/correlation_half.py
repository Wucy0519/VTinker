...
def FunctionCorrelation(tenFirst, tenSecond):
	return _FunctionCorrelation.apply(tenFirst.float(), tenSecond.float()).half()
# end
...
def CorrelationB(fmap0, fmap1, flow, coord):
	coord = coord + flow
	coord = coord.permute(0,2,3,1)
	b,c,h,w = fmap1.shape
	r = 4
	k = 2 * r + 1
	dx = torch.linspace(-r, r, 2*r+1, device=coord.device)
	dy = torch.linspace(-r, r, 2*r+1, device=coord.device)
	delta = torch.stack(torch.meshgrid(dy, dx, indexing='ij'), axis=-1)
	delta_lvl = delta.view(1, k*k, 1, 1, 2).expand(b,-1,h,w,2)

	coord = coord.reshape(b,1,h,w,2)
	coord = (coord + delta_lvl).view(-1,h,w,2)
	fmap0 = fmap0.view(b,1,c,h,w).expand(-1,k*k,-1,-1,-1).reshape(-1,c,h,w)
	fmap1 = fmap1.view(b,1,c,h,w).expand(-1,k*k,-1,-1,-1).reshape(-1,c,h,w)
	coor = bilinear_sampler(fmap1,coord)
	volume = (fmap0 * coor).sum(dim = 1).view(b,k*k,h,w)

	return volume


def CorrelationM(fmap0, fmap1, flow, coord):
	t = 0.5
	coord0 = coord - flow * t
	coord1 = coord + flow * (1-t)
	coord0 = coord0.permute(0,2,3,1)
	coord1 = coord1.permute(0,2,3,1)
	b,c,h,w = fmap1.shape
	r = 4
	k = 2 * r + 1
	dx = torch.linspace(-r, r, 2*r+1, device=coord.device)
	...
	coord0 = coord0.reshape(b,1,h,w,2)
	coord0 = (coord0 - delta_lvl * t).view(-1,h,w,2)
	coord1 = coord1.reshape(b,1,h,w,2)
	coord1 = (coord1 + delta_lvl * (1-t)).view(-1,h,w,2)
	fmap0 = fmap0.view(b,1,c,h,w).expand(-1,k*k,-1,-1,-1).reshape(-1,c,h,w)
	fmap1 = fmap1.view(b,1,c,h,w).expand(-1,k*k,-1,-1,-1).reshape(-1,c,h,w)
	coor0 = bilinear_sampler(fmap0,coord0)
	coor1 = bilinear_sampler(fmap1,coord1)
	volume = (coor0 * coor1).sum(dim = 1).view(b,k*k,h,w)
