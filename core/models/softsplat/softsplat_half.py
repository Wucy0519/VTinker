...
def cupy_kernel(strFunction, objVariables):
	...
	while True:
		...
		strIndex = [ '((' + strArgs[intArg + 1].replace('{', '(').replace('}', ')')\
                .strip() + ')*' + str(intStrides[intArg]) + ')' for intArg in range(intArgs) ]
...
class _FunctionSoftsplat(torch.autograd.Function):
	...
	def backward(self, gradOutput):
		...
		gradFlow = input.new_zeros([ intSamples, intFlowDepth, intFlowHeight, intFlowWidth ])\
                if self.needs_input_grad[1] == True else None
