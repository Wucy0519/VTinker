...
from ..utils import correlation,matching
...
class MotionEstimator(nn.Module):
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        feat0_warp = warp(feat1, last_flow[:, :2]*0.25)
        feat1_warp = warp(feat0, last_flow[:, 2:]*0.25)
        ...
        input_feat0 = torch.cat([volume0, feat0, feat0_warp,last_feat[:,:64],last_flow[:,:2]], 1)
        input_feat1 = torch.cat([volume1, feat1, feat1_warp,last_feat[:,64:],last_flow[:,2:]], 1)
        ...
        b,_,_,_ = flow.shape
        flow = torch.cat([flow[:b//2],flow[b//2:]],dim=1)
        feat = torch.cat([feat[:b//2],feat[b//2:]],dim=1)
        return flow, feat
...
class Model(nn.Module):
    ...
    def forward_one_lvl(self,
        ...
        if last_flow is None:
            last_flow,_ = matching.global_correlation_softmax(feat0_pyr[-1], feat1_pyr[-1],True)
            N, _, H, W = last_flow.shape
            last_flow = torch.cat([last_flow[:N//2],last_flow[N//2:]],dim=1).contiguous() * 4
